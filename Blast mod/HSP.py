...
def CheckOverlap(hsp,gene,Query):
    ...
    while (ind<size): # iterate on items to remove common overlap
        ...
        if FrightIndGene >=SleftIndGene :
            ...
            if hsp1 <= hsp2:
                FleftQuery = hsp1
                FrightQuery = hsp[ind][2]
                SleftQuery = hsp2
                SrightQuery = hsp[ind+1][2]
            else: 
                SleftQuery = hsp1
                SrightQuery = hsp[ind][2]
                FleftQuery = hsp2
                FrightQuery = hsp[ind+1][2]
            ...
            # seq2 --> ....PQEG.....
                ...
                leftGene = min (FleftIndGene,SleftIndGene)
                rightGene = max(FrightIndGene,SrightIndGene)
                newHspListFinal.append ([rightGene,leftGene,rightQuery,leftQuery,max (hsp[ind][4],hsp[ind+1][4])])
                ind +=1
